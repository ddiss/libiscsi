...
void fill_xcopy_queue(struct client *client)
{
	while (client->in_flight < max_in_flight && client->pos < client->src_num_blocks) {
		...
		/* TODO check that two CSCD descs are supported */
		...
		if (task == NULL) {
			printf("failed to alloc XCOPY command\n");
			exit(10);
		}

		if (iscsi_scsi_command_async(client->src_iscsi, client->src_lun,
					     task, xcopy_cb,
					     &data, client) != 0) {
		      ...
		      int lun)
{
	struct scsi_task *task = NULL;
        ...
        task = scsi_cdb_receive_copy_results(SCSI_COPY_RESULTS_OP_PARAMS, 0, 1024);
	if (task == NULL) {
		exit(10);
	}
